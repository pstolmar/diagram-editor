...
/** Curated landscape/scenic photo sets by category */
const PHOTO_SETS = {
  ocean: [
...
function resolvePhotoCategory(row) {
  // Row value OR ?photos=xxx query param
...
function applyEntrance(slot, delay) {
  ...
  slot.offsetWidth;
  // Always reset delay so cycling slots don't inherit the initial stagger value
...
export default function decorate(block) {
  ...
  const imageRows = photoCategory ? [] : rows.slice(photoCategory ? 3 : 4);
