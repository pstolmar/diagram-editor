...
function getRadius(mode) {
  if (mode === 'dots') return 4 + Math.random() * 8;
  if (mode === 'stars') return 0.8 + Math.random() * 2.2;
...
export default async function decorate(block) {
  ...
  const palette = PALETTES[colorRaw] || PALETTES.blue;
  const count = Math.min(Math.max(parseInt(countRaw, 10) || 90, 24), 220);
    ...
    state.particles.forEach((pt) => {
      ...
      if (mode === 'fire') {
        ...
        if (mode === 'stars') {
          ...
          ctx.fillStyle = `rgba(${pt.rgb.r},${pt.rgb.g},${pt.rgb.b},${pt.opacity})`;
        }
      }
