...
export default async function decorate(block) {
  ...
  // Track total swaps to trigger whole-carousel re-entrance periodically
  let swapCount = 0;
  // Trigger full re-entrance every time all slots have been replaced (~2 full rounds)
  const FULL_ENTRANCE_EVERY = slotCount * 2;
  const STAGGER_MS = 200;
  const ANIM_DURATION_MS = 1600;

  const triggerFullEntrance = () => {
    ...
    const timeoutId = window.setTimeout(() => {
      ...
      const doSwap = () => {
        ...
        if (next) {
          ...
          // Every FULL_ENTRANCE_EVERY swaps: re-entrance all slots simultaneously
          if (swapCount % FULL_ENTRANCE_EVERY === 0) {
            triggerFullEntrance();
          } else {
            // Individual slot entrance — always reset --pc-delay to 0s
